    ...
    def notes_to_midi(self, file: MIDIFile) -> None:
        ...
        for i, pitch in enumerate(array_of_notes_numbers):
            if pitch > -1:
                ...
                file.addNote(self.track, self.channel, pitch,
                             self.start_time + Melody.__get_time_offset(notes_duration, i),
                             self.notes[i].duration.value, self.volume)
...
def notes_to_midi(file: MIDIFile, array_of_notes: list[str], notes_duration: list[float], octave=4, start_time=0, track=0,
                  channel=0, volume=100) -> None:
    """Save the sequence of notes in the MIDI file"""
    ...
    for i, pitch in enumerate(array_of_notes_numbers):
        file.addNote(track, channel, pitch, start_time + __get_time_offset(notes_duration, i), notes_duration[i],
                     volume)
